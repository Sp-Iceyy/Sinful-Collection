...
const themeKey = 'framehouse-theme';
const customThemeColorKey = 'framehouse-custom-color';
const backgroundKey = 'framehouse-background';
const themeOptions = ['pink', 'dark-blue', 'light-blue', 'red', 'green', 'dark-green', 'purple'];
const oldDemoTitles = ['Quiet Geometry', 'Pacific / 04', 'Citrus Study', 'Sunday Table', 'Soft Focus', 'Red Thread'];
let users = {}, currentUserId = localStorage.getItem(activeUserKey) || '', media = [], activeFilter = 'all', activeTags = [], removingFavoriteTags = false, removingMedia = false, selectedMediaIds = [], batchFiles = [], activeArtist = '', activeArtistEdit = null, activeView = 'library', activeFolder = '', editingId = null, safeMode = localStorage.getItem('framehouse-safe-mode') === 'true', activeTheme = (themeOptions.includes(localStorage.getItem(themeKey)) || localStorage.getItem(themeKey) === 'custom') ? localStorage.getItem(themeKey) : 'pink', customThemeColor = localStorage.getItem(customThemeColorKey) || '#ff4f9a', activeBackground = localStorage.getItem(backgroundKey) || '';
...
function saveCatalog() { const currentUser = getCurrentUser(); if (!currentUser) return Promise.resolve(); return fetch('/api/catalog', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile: currentUserId, media: currentUser.media, favoriteTags: currentUser.favoriteTags || [], artists: currentUser.artists || [], folders: currentUser.folders || [], theme: currentUser.theme || 'pink', customColor: currentUser.customColor || customThemeColor, backgroundImage: currentUser.backgroundImage || '' }) }).catch(() => {}); }
async function loadUsers() {
  try {
    const response = await fetch('/api/catalog?profile=');
    const catalog = response.ok ? await response.json() : { profiles: {} };
    users = Object.fromEntries(Object.entries(catalog.profiles || {}).map(([id, value]) => [id, { id, username: value.username || id, media: (value.media || []).map(normalize), favoriteTags: Array.isArray(value.favoriteTags) ? value.favoriteTags : [], artists: Array.isArray(value.artists) ? value.artists : [], folders: Array.isArray(value.folders) ? value.folders : [], theme: (themeOptions.includes(value.theme) || value.theme === 'custom') ? value.theme : 'pink', customColor: typeof value.customColor === 'string' ? value.customColor : '#ff4f9a', backgroundImage: typeof value.backgroundImage === 'string' ? value.backgroundImage : '' }]));
  } catch { users = {}; }
  renderGateAccounts(); renderAccountMenu();
  const activeUser = getCurrentUser();
  if (activeUser) { applyTheme(activeUser.theme, { persist: false, customColor: activeUser.customColor }); applyBackground(activeUser.backgroundImage, { persist: false }); }
}
function setActiveUser(userId) {
  ...
  applyTheme(getCurrentUser()?.theme, { persist: false, customColor: getCurrentUser()?.customColor });
  applyBackground(getCurrentUser()?.backgroundImage || '', { persist: false });
...
function resolveStoredFilename(item) { if (item?.filename) return item.filename; const raw = item?.src?.split('/').pop() || ''; try { return decodeURIComponent(raw); } catch { return raw; } }
async function deleteProfileFiles(profileUser) {
  ...
  const deletions = profileUser.media.map(async (item) => {
    const safeName = resolveStoredFilename(item);
    if (!safeName) return;
    try {
...
async function deleteStoredMedia(item) { const filename = resolveStoredFilename(item); if (!filename) return; const response = await fetch('/api/media/file', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename, folder: item.type === 'video' ? 'Videos' : 'Images' }) }); if (!response.ok && response.status !== 404) throw new Error('The media file could not be removed from storage.'); }
...
function logoutUser() {
  ...
  activeView = 'library';
  activeFolder = '';
  applyTheme(localStorage.getItem(themeKey), { persist: false, customColor: localStorage.getItem(customThemeColorKey) });
  applyBackground(localStorage.getItem(backgroundKey) || '', { persist: false });
  renderGateAccounts();
  renderAccountMenu();
...
function hexToRgbString(hex) { const clean = String(hex || '').replace('#', ''); const full = clean.length === 3 ? clean.split('').map((char) => char + char).join('') : clean.padEnd(6, '0'); const value = parseInt(full, 16) || 0; return `${(value >> 16) & 255},${(value >> 8) & 255},${value & 255}`; }
function lightenHex(hex, amount = 0.35) { const clean = String(hex || '').replace('#', ''); const full = clean.length === 3 ? clean.split('').map((char) => char + char).join('') : clean.padEnd(6, '0'); const value = parseInt(full, 16) || 0; const mix = (channel) => Math.round(channel + (255 - channel) * amount); const r = mix((value >> 16) & 255), g = mix((value >> 8) & 255), b = mix(value & 255); return `#${[r, g, b].map((channel) => channel.toString(16).padStart(2, '0')).join('')}`; }
function applyTheme(theme, options = {}) { const persist = options.persist !== false; if (theme === 'custom') customThemeColor = options.customColor || getCurrentUser()?.customColor || customThemeColor; const next = theme === 'custom' ? 'custom' : (themeOptions.includes(theme) ? theme : 'pink'); activeTheme = next; themeOptions.forEach((option) => document.body.classList.toggle(`theme-${option}`, option !== 'pink' && option === next)); if (next === 'custom') { document.body.style.setProperty('--pink', customThemeColor); document.body.style.setProperty('--soft-pink', lightenHex(customThemeColor)); document.body.style.setProperty('--pink-rgb', hexToRgbString(customThemeColor)); } else { document.body.style.removeProperty('--pink'); document.body.style.removeProperty('--soft-pink'); document.body.style.removeProperty('--pink-rgb'); } document.querySelectorAll('.theme-option').forEach((button) => button.classList.toggle('active', button.dataset.theme === next)); if ($('#custom-color-input')) $('#custom-color-input').value = customThemeColor; if ($('#custom-theme-swatch')) $('#custom-theme-swatch').style.background = customThemeColor; if (!persist) return; const user = getCurrentUser(); if (user) { user.theme = next; user.customColor = customThemeColor; saveCatalog(); } else { localStorage.setItem(themeKey, next); localStorage.setItem(customThemeColorKey, customThemeColor); } }
function applyBackground(src, options = {}) { const persist = options.persist !== false; activeBackground = src || ''; const hasImage = Boolean(activeBackground); document.body.classList.toggle('has-bg-image', hasImage); const blurLayer = $('#bg-blur-layer'); const fitLayer = $('#bg-fit-layer'); const imageValue = hasImage ? `url("${activeBackground}")` : ''; if (blurLayer) blurLayer.style.backgroundImage = imageValue; if (fitLayer) fitLayer.style.backgroundImage = imageValue; if ($('#reset-background')) $('#reset-background').hidden = !hasImage; if (!persist) return; const user = getCurrentUser(); if (user) { user.backgroundImage = activeBackground; saveCatalog(); } else localStorage.setItem(backgroundKey, activeBackground); }
...
function render() {
  ...
  const filtered = media.filter((item) => {
    ...
    const matchesFolder = !activeFolder ? true : item.folder === activeFolder;
    const matchesArtist = !activeArtist || item.artist.toLowerCase() === activeArtist.toLowerCase();
    const matchesFavorite = activeView !== 'favorites' || item.favorite;
    const matchesSearch = !query || [item.title, item.artist, item.description, ...item.tags].join(' ').toLowerCase().includes(query);
    return matchesFilter && matchesFolder && matchesArtist && matchesFavorite && matchesSearch;
  ...
  $('#all-count').textContent = media.length; $('#image-count').textContent = media.filter((item) => item.type === 'image').length; $('#video-count').textContent = media.filter((item) => item.type === 'video').length; $('#favorite-count').textContent = media.filter((item) => item.favorite).length;
  renderFavoriteTags();
  $('#page-title').innerHTML = activeFolder ? `${activeFolder}<span class="accent">.</span>` : activeView === 'favorites' ? 'Favorites<span class="accent">.</span>' : 'Sinful Collection<span class="accent">.</span>'; $('#view-eyebrow').textContent = activeFolder ? 'CURRENTLY VIEWING FOLDER' : activeView === 'favorites' ? 'THE PIECES YOU KEPT CLOSE' : 'YOUR PRIVATE OBSESSION'; $('#page-subhead').textContent = activeFolder ? 'Everything in this folder.' : activeView === 'favorites' ? 'Tap the heart on any piece to keep it close.' : 'Your own world for the depraved.';
  $('#safe-mode').classList.toggle('active', safeMode); $('#safe-mode').setAttribute('aria-pressed', String(safeMode)); $('#safe-mode').innerHTML = `<span aria-hidden="true">${safeMode ? '✓' : '◉'}</span> Safe Mode${safeMode ? ': ON' : ''}`; document.body.classList.toggle('safe-mode-active', safeMode);
}
...
function openEditor(id = null) { editingId = id; const item = media.find((entry) => entry.id === id); $('#add-form').reset(); batchThumbUrls.forEach((url) => URL.revokeObjectURL(url)); batchThumbUrls = []; $('#add-batch-list').innerHTML = ''; renderFolderOptions(item ? getFolderName(item) : 'Unsorted'); $('#media-file').required = !item; $('#file-label').textContent = item ? 'Keep current file or choose a replacement' : 'Choose images or videos'; setAddMode(false); if (item) { $('#media-title').value = item.title; $('#media-artist').value = item.artist; $('#media-tags').value = item.tags.join(', '); $('#media-description').value = item.description; $('#media-folder').value = getFolderName(item); $('#media-folder-custom').value = ''; $('#media-nsfw').checked = item.tags.includes('nsfw'); } else { $('#media-artist').value = localStorage.getItem(lastArtistKey) || ''; } $('#add-modal').hidden = false; }
function setAddMode(isBatch, count = 0) { $('#add-single-fields').hidden = isBatch; $('#media-title').required = !isBatch; $('#apply-batch-artist').hidden = !isBatch; $('#add-batch-list').hidden = !isBatch; $('#add-modal .add-modal').classList.toggle('batch-mode', isBatch); $('#form-eyebrow').textContent = isBatch ? 'BATCH IMPORT' : (editingId ? 'EDIT TEMPTATION' : 'NEW TEMPTATION'); $('#add-title').innerHTML = isBatch ? 'Import media<span class="accent">.</span>' : (editingId ? 'Edit media<span class="accent">.</span>' : 'Add to collection<span class="accent">.</span>'); $('#form-submit').innerHTML = isBatch ? `Import ${count} file${count === 1 ? '' : 's'} <span>↗</span>` : (editingId ? 'Save changes <span>↗</span>' : 'Add to collection <span>↗</span>'); $('.folder-pick-row').hidden = isBatch || Boolean(editingId); }
let batchThumbUrls = [];
function renderAddBatchRows(files) { batchThumbUrls.forEach((url) => URL.revokeObjectURL(url)); batchThumbUrls = files.map((file) => URL.createObjectURL(file)); batchFiles = files; const folders = ['Unsorted', ...getFolderOptions().filter((folder) => folder !== 'Unsorted')]; $('#add-batch-list').innerHTML = files.map((file, index) => `<div class="batch-file-row" data-index="${index}"><div class="batch-file-head"><span class="batch-thumb">${file.type.startsWith('video/') ? `<video src="${batchThumbUrls[index]}" muted></video>` : `<img src="${batchThumbUrls[index]}" alt="">`}</span><span class="batch-file-name"></span></div><input name="title" aria-label="Title" placeholder="Title" value="${file.name.replace(/\.[^/.]+$/, '').replace(/"/g, '&quot;')}" required><input name="artist" aria-label="Artist" placeholder="Artist" value="${(localStorage.getItem(lastArtistKey) || '').replace(/"/g, '&quot;')}"><input name="tags" aria-label="Tags" placeholder="Tags"><select name="folder" aria-label="Folder">${folders.map((folder) => `<option value="${folder}">${folder}</option>`).join('')}</select><input name="custom-folder" aria-label="New folder" placeholder="Or create folder"><label class="batch-nsfw"><input name="nsfw" type="checkbox"> NSFW</label><textarea name="description" rows="1" aria-label="Description" placeholder="Description"></textarea></div>`).join(''); files.forEach((file, index) => { $('#add-batch-list').querySelector(`[data-index="${index}"] .batch-file-name`).textContent = file.name; }); $('#apply-batch-artist').onclick = () => { const artist = $('#media-artist').value.trim(); if (artist) { $('#add-batch-list').querySelectorAll('[name="artist"]').forEach((input) => { input.value = artist; }); localStorage.setItem(lastArtistKey, artist); } }; setAddMode(true, files.length); }
...
$('#media-grid').addEventListener('click', async (event) => {
  const card = event.target.closest('.media-card'); const folderCard = event.target.closest('.folder-card'); if (removingMedia && (card || folderCard)) { event.preventDefault(); event.stopPropagation(); const ids = folderCard?.dataset.mediaIds ? folderCard.dataset.mediaIds.split(',').filter(Boolean) : card?.dataset.id ? [card.dataset.id] : []; const shouldSelect = ids.some((id) => !selectedMediaIds.includes(id)); selectedMediaIds = shouldSelect ? [...new Set([...selectedMediaIds, ...ids])] : selectedMediaIds.filter((id) => !ids.includes(id)); updateRemovalSelection(); return; }
  if (folderCard && event.target.closest('[data-action="rename-folder"]')) { event.stopPropagation(); const folder = folderCard.dataset.folder; if (folder) { renamingFolder = folder; $('#folder-form').reset(); $('#folder-name-input').value = folder; $('#folder-modal-title').innerHTML = 'Rename folder<span class="accent">.</span>'; $('#folder-form-submit').innerHTML = 'Save changes <span>↗</span>'; $('#folder-modal').hidden = false; $('#folder-name-input').focus(); } return; }
...
$('#search-input').addEventListener('input', render); $('#sort-select').addEventListener('change', render); $('#open-add').addEventListener('click', () => openEditor()); $('#empty-add').addEventListener('click', () => openEditor()); document.querySelectorAll('[data-close]').forEach((button) => button.addEventListener('click', () => closeModal(button.dataset.close))); document.querySelectorAll('.modal-backdrop').forEach((backdrop) => backdrop.addEventListener('click', (event) => { if (event.target === backdrop) closeModal(backdrop.id); })); document.addEventListener('keydown', (event) => { if (event.key === 'Escape') document.querySelectorAll('.modal-backdrop:not([hidden])').forEach((modal) => closeModal(modal.id)); }); $('#media-file').addEventListener('change', (event) => { const files = [...event.target.files]; if (files.length > 1) { $('#file-label').textContent = `${files.length} files selected`; renderAddBatchRows(files); return; } $('#file-label').textContent = files[0]?.name || (editingId ? 'Keep current file or choose a replacement' : 'Choose images or videos'); setAddMode(false); if (files[0] && !editingId) $('#media-title').value = files[0].name.replace(/\.[^/.]+$/, ''); });
$('#choose-folder-button').addEventListener('click', () => { $('#media-folder-file').click(); });
$('#media-folder-file').addEventListener('change', (event) => { const files = [...event.target.files].filter((file) => file.type.startsWith('image/') || file.type.startsWith('video/')); if (files.length) { renderAddBatchRows(files); const folderName = files[0].webkitRelativePath?.split('/')[0] || ''; if (folderName) $('#add-batch-list').querySelectorAll('[name="custom-folder"]').forEach((input) => { input.value = folderName; }); } event.target.value = ''; });
...
function readEntryFiles(entry) {
  return new Promise((resolve) => {
    if (entry.isFile) { entry.file((file) => resolve([file]), () => resolve([])); return; }
    if (!entry.isDirectory) { resolve([]); return; }
    const reader = entry.createReader(); const collected = [];
    const readBatch = () => reader.readEntries(async (entries) => { if (!entries.length) { resolve(collected); return; } for (const child of entries) collected.push(...await readEntryFiles(child)); readBatch(); }, () => resolve(collected));
    readBatch();
  });
}
async function handleDroppedFiles(dataTransfer) {
  const items = dataTransfer.items ? [...dataTransfer.items].filter((item) => item.kind === 'file') : [];
  const entries = items.map((item) => item.webkitGetAsEntry && item.webkitGetAsEntry()).filter(Boolean);
  let files = []; let folderName = '';
  if (entries.length) { for (const entry of entries) { files.push(...await readEntryFiles(entry)); if (entry.isDirectory && !folderName) folderName = entry.name; } }
  else files = [...dataTransfer.files];
  const mediaFiles = files.filter((file) => file.type.startsWith('image/') || file.type.startsWith('video/'));
  if (!mediaFiles.length) return;
  const isFolderDrop = entries.some((entry) => entry.isDirectory);
  openEditor();
  if (!isFolderDrop && mediaFiles.length === 1) { const transfer = new DataTransfer(); transfer.items.add(mediaFiles[0]); $('#media-file').files = transfer.files; $('#media-file').dispatchEvent(new Event('change')); return; }
  renderAddBatchRows(mediaFiles);
  if (isFolderDrop && folderName) $('#add-batch-list').querySelectorAll('[name="custom-folder"]').forEach((input) => { input.value = folderName; });
}
let dragDepth = 0;
document.addEventListener('dragenter', (event) => { if (!document.body.classList.contains('unlocked')) return; event.preventDefault(); dragDepth++; document.body.classList.add('drag-active'); });
document.addEventListener('dragover', (event) => { if (!document.body.classList.contains('unlocked')) return; event.preventDefault(); });
document.addEventListener('dragleave', (event) => { if (!document.body.classList.contains('unlocked')) return; dragDepth = Math.max(0, dragDepth - 1); if (!dragDepth) document.body.classList.remove('drag-active'); });
document.addEventListener('drop', async (event) => { if (!document.body.classList.contains('unlocked')) return; event.preventDefault(); dragDepth = 0; document.body.classList.remove('drag-active'); await handleDroppedFiles(event.dataTransfer); });
...
const updateRepo = 'Sp-Iceyy/Sinful-Collection';
function parseVersion(value) { return String(value || '').trim().replace(/^v/i, '').split('.').map((part) => parseInt(part, 10) || 0); }
function isNewerVersion(remote, local) { const r = parseVersion(remote), l = parseVersion(local); for (let i = 0; i < Math.max(r.length, l.length); i++) { const rv = r[i] || 0, lv = l[i] || 0; if (rv !== lv) return rv > lv; } return false; }
async function checkForUpdates() {
  const button = $('#check-for-updates'); const originalText = button.textContent;
  button.disabled = true; button.textContent = 'Checking…';
  try {
    const currentVersion = (await window.electronAPI?.getAppVersion?.()) || $('#app-version')?.textContent || '0.0.0';
    const response = await fetch(`https://api.github.com/repos/${updateRepo}/releases/latest`, { headers: { Accept: 'application/vnd.github+json' } });
    if (!response.ok) throw new Error(response.status === 404 ? 'No releases have been published yet.' : 'Unable to check for updates right now.');
    const release = await response.json();
    const latestVersion = release.tag_name || release.name || '';
    if (isNewerVersion(latestVersion, currentVersion)) {
      if (window.confirm(`A new version is available: ${latestVersion} (you have v${parseVersion(currentVersion).join('.')}).\n\nOpen the release page?`)) {
        if (window.electronAPI?.openExternal) window.electronAPI.openExternal(release.html_url);
        else window.open(release.html_url, '_blank', 'noopener,noreferrer');
      }
    } else {
      window.alert(`You're up to date (v${parseVersion(currentVersion).join('.')}).`);
    }
  } catch (error) {
...
$('#check-for-updates').addEventListener('click', () => { toggleHelpMenu(false); checkForUpdates(); });
$('#themes-toggle').addEventListener('click', () => { applyTheme(activeTheme, { persist: false }); $('#theme-modal').hidden = false; });
$('#custom-color-input').addEventListener('input', (event) => { applyTheme('custom', { customColor: event.target.value, persist: false }); });
$('#custom-color-input').addEventListener('change', (event) => { applyTheme('custom', { customColor: event.target.value }); });
$('#change-background').addEventListener('click', () => { $('#background-file-input').click(); });
$('#background-file-input').addEventListener('change', async (event) => { const file = event.target.files[0]; event.target.value = ''; if (!file) return; try { const uploaded = await uploadToMedia(file); applyBackground(uploaded.src); } catch (error) { window.alert(error.message || 'The background image could not be applied.'); } });
$('#reset-background').addEventListener('click', () => { applyBackground(''); });
$('#theme-grid').addEventListener('click', (event) => { const button = event.target.closest('.theme-option'); if (!button) return; applyTheme(button.dataset.theme); });
applyTheme(activeTheme);
applyBackground(activeBackground, { persist: false });
...
$('#erase-all-media').addEventListener('click', () => { $('#erase-confirm-modal').hidden = false; });
$('#erase-confirm-no').addEventListener('click', () => { closeModal('erase-confirm-modal'); });
$('#erase-confirm-yes').addEventListener('click', async () => { closeModal('erase-confirm-modal'); const items = [...media]; try { await Promise.all(items.map(deleteStoredMedia)); } catch (error) { window.alert(error.message); } media = []; selectedMediaIds = []; await saveMedia(); render(); });
let renamingFolder = null;
$('#add-empty-folder').addEventListener('click', () => { renamingFolder = null; $('#folder-form').reset(); $('#folder-modal-title').innerHTML = 'Name this folder<span class="accent">.</span>'; $('#folder-form-submit').innerHTML = 'Create folder <span>↗</span>'; $('#folder-modal').hidden = false; $('#folder-name-input').focus(); });
$('#folder-form').addEventListener('submit', async (event) => { event.preventDefault(); const name = $('#folder-name-input').value.trim(); const user = getCurrentUser(); if (!user || !name) return; const isDuplicate = name.toLowerCase() === 'unsorted' || getFolderOptions().some((folder) => folder.toLowerCase() === name.toLowerCase() && folder.toLowerCase() !== renamingFolder?.toLowerCase()); if (isDuplicate) { window.alert('That folder already exists.'); return; } if (renamingFolder) { const previous = renamingFolder; if (name !== previous) { user.folders = (user.folders || []).map((entry) => entry === previous ? name : entry); media.forEach((item) => { if (getFolderName(item) === previous) item.folder = name; }); if (activeFolder === previous) activeFolder = name; await saveMedia(); } } else { user.folders = [...(user.folders || []), name].sort((a, b) => a.localeCompare(b)); await saveCatalog(); } renamingFolder = null; closeModal('folder-modal'); render(); });
...
$('#set-card-background').addEventListener('click', () => { cardContextMenu.hidden = true; if (contextMediaItem?.type === 'image') applyBackground(contextMediaItem.src); contextMediaItem = null; });
initPasswordGate();
loadUsers();
